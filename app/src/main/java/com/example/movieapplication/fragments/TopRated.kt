...
import com.example.movieapplication.services.MovieTopApiInterface
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class TopRated : Fragment(), MovieAdapter.RecyclerViewClickInterface {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<RecyclerView>(R.id.rv_top_rated_movies_list).layoutManager = LinearLayoutManager(context)
        view.findViewById<RecyclerView>(R.id.rv_top_rated_movies_list).setHasFixedSize(true)
        getMovieData { movies : List<Movie> ->
            myAdapter = MovieAdapter(movies, this)
            view.findViewById<RecyclerView>(R.id.rv_top_rated_movies_list).adapter = myAdapter
        }
    }

    private fun getMovieData(callback: (List<Movie>) -> Unit){
        val apiService = MovieApiService.getInstance().create(MovieTopApiInterface::class.java)
