...
import com.example.movieapplication.services.MovieApiInterface
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class Popular : Fragment() , MovieAdapter.RecyclerViewClickInterface {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<RecyclerView>(R.id.rv_popular_movies_list).layoutManager = LinearLayoutManager(context)
        view.findViewById<RecyclerView>(R.id.rv_popular_movies_list).setHasFixedSize(true)
        getMovieData { movies : List<Movie> ->
            myAdapter = MovieAdapter(movies, this)
            view.findViewById<RecyclerView>(R.id.rv_popular_movies_list).adapter = myAdapter
        }
    }
